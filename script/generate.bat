...
SET ENV=fi.tnie.db.meta.impl.pg.PGEnvironment
SET ROOT_PACKAGE=fi.tnie.db.build
...
REM read to go:
java -classpath %META_GEN_CP% fi.tnie.db.build.Builder -g %GENSRC% -j %JDBC_CONFIG% -e %ENV% --root-package %ROOT_PACKAGE% -u %JDBC_URL%
@echo on